...
stock_barang = baca_stok(nama_file)
...
def tampilkan_data(stok_dict):
    ...
    print(f"{'Kode' : <10} | {'Barang' : <15} | {'Jumlah' :>3}")
    print("-" * 37) #membuat garis panjang secara otomatis

    #menampilkan isi data
    for kode in sorted(stok_dict.keys()):
        barang = stok_dict[kode]["barang"]
        jumlah = stok_dict[kode]["jumlah"]
        print(f"{kode:<10} | {barang:<15} | {int(jumlah):>6}")
...
def main():
    ...
    while True:
        ...
        if pilihan == "1":
            tampilkan_data(stock_barang)
        elif pilihan == "2":
            cari_barang(stock_barang)
        elif pilihan == "3":
            tambah_barang(stock_barang)
        elif pilihan == "4":
            update_stok(stock_barang)
        elif pilihan == "5":
            simpan_stok(nama_file, stock_barang)
